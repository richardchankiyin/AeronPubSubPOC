...
public class RecordedBasicPublisher
{
	...
	private static final Logger log = LoggerFactory.getLogger(RecordedBasicPublisher.class);
    ...
    public static void publish(String channelID, int streamID, long noOfMessages) throws InterruptedException
    {
    	log.info("Publishing to {} on stream id {}", channelID, streamID);
        ...
        {
            archive.startRecording(channelID, streamID, SourceLocation.LOCAL);

            try (Publication publication = archive.context().aeron().addPublication(channelID, streamID))
            {
                ...
                for (long i = 0; i < noOfMessages && running.get(); i++)
                {
                    final String message = "Hello World! " + i;
                    final byte[] messageBytes = message.getBytes();
                    BUFFER.putBytes(0, messageBytes);

                    log.info("Offering {}/{} - ", i , noOfMessages);
            ...
            finally
            {
                ...
                archive.stopRecording(channelID, streamID);
            }
        }
    }
    ...
    private static void checkResult(final long position)
    {
        ...
        {
        	log.warn("Offer failed due to back pressure");
        }
        else if (position == Publication.ADMIN_ACTION)
        {
            log.warn("Offer failed because of an administration action in the system");
        }
        else if (position == Publication.NOT_CONNECTED)
        {
            log.warn("Offer failed because publisher is not connected to subscriber");
        }
        else if (position == Publication.CLOSED)
        {
            log.warn("Offer failed publication is closed");
        }
        ...
        {
            log.warn("Offer failed due to unknown result code: {}", position);
        }
    }
}